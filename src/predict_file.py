import re
import pandas as pd
from inference import predict  # ⚡ берём predict из inference.py (он уже с ансамблем)
...
def split_entity_into_bio_spans(text: str, start: int, end: int, ent_type: str):
    """
    Разбивает entity-спан на BIO-метки:
    - первое слово → B-<TYPE>
    - остальные → I-<TYPE>
    """
...
def predict_file(input_path: str, output_path: str):
    ...
    for text in df["sample"]:
        entities = predict(text)  # ⚡ теперь предсказания от ансамбля
        spans = []
        for ent in entities:
